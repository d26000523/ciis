...
public class MqttService extends Service implements MqttTraceHandler {
    ...
    public int onStartCommand(final Intent intent, int flags, final int startId) {
        ...
        // process restarted
        try {
            SocketUDPClient socketUDPClient = new SocketUDPClient(this);
    ...
    public class SocketUDPClient {
        ...
        public SocketUDPClient(Context context) throws UnknownHostException {
            //初始化(和Server連接)
            try {
                datasock = new DatagramSocket(20002);
                ...
                public void run() {
                    try {
                        while(true) {
                            ...
                            Log.d("connectServer", "recieveMessage");
                            String[] msg = new String(Datapack.getData(), Datapack.getOffset(),
                                    Datapack.getLength()).split(":");
                            if (tag == 0) {
                                AnotherClientAddr = msg[0];

                                notif();
                                //取得通知管理器
                                //      AnotherClientPort = Integer.parseInt(msg[1]);
        ...
        public void notif(){
            ...
            SharedSocket sharedSocket = (SharedSocket)context.getApplicationContext();
            sharedSocket.mqttreceivecall = AnotherClientAddr;
            ...
            Intent call = new Intent(context, MqttTalkingIntent.class);
            //非必要,可以利用intent傳值
